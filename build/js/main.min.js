...
/* harmony import */ var _modules_init_intro_slider_js__WEBPACK_IMPORTED_MODULE_5__ = __webpack_require__(/*! ./modules/init-intro-slider.js */ "./js/modules/init-intro-slider.js");
...
window.addEventListener('DOMContentLoaded', function () {
  ...
  window.addEventListener('load', function () {
    ...
    burger.init();
    Object(_modules_init_intro_slider_js__WEBPACK_IMPORTED_MODULE_5__["initIntroSlider"])();
  });
});
...
var sliderSwiper = document.querySelector('.intro__slider');
var swiper;
...
var initIntroSlider = function initIntroSlider() {
  ...
  var slides = sliderSwiper.querySelectorAll('.swiper-slide');
  var getRandomInt = function getRandomInt(min, max) {
    min = Math.ceil(min);
    max = Math.floor(max);
    return Math.floor(Math.random() * (max - min + 1)) + min;
  };
  var shuffle = function shuffle(someArray) {
    var newSomeArray = someArray.slice();
    for (var i = newSomeArray.length - 1; i > 0; i--) {
      var randomPosition = Math.floor(Math.random() * i);
      var _ref = [newSomeArray[randomPosition], newSomeArray[i]];
      newSomeArray[i] = _ref[0];
      newSomeArray[randomPosition] = _ref[1];
    }
    return newSomeArray;
  };
  var getNewArray = function getNewArray(array, numberLength) {
    return shuffle(array).slice(0, getRandomInt(1, numberLength));
  };
  var fillColorItems = function fillColorItems() {
    var items = getNewArray(Array.from(slides), 12);
    items.forEach(function (item) {
      return item.classList.add('fill-color');
    });
    setTimeout(function () {
      return items.forEach(function (item) {
        return item.classList.remove('fill-color');
      });
    }, 1450);
  };
